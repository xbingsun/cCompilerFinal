...
void Praser::praser_jump_statement(struct gramTree* node) {
    if (node->left->name == "GOTO") {
        ...
        innerCode.addCode("(j, _, _, " + blockStack[num].breakLabelname + ")");
    }
...
void Praser::praser_iteration_statement(struct gramTree* node) {
    if (node->left->name == "WHILE") {
        ...
        if (var.type == "bool") {
            innerCode.addCode("(" + var.boolString + ", " + label2+")");
        }
        else {
            string tempzeroname = "temp" + inttostr(innerCode.tempNum);
            ++innerCode.tempNum;
        ...
        innerCode.addCode("(j, _, _, " + label1 + ")");
        innerCode.addCode("LABEL " + label3 + " :");
        ...
        if (var.type == "bool") {
            innerCode.addCode("(" + var.boolString + ", " + label1+")");
        }
        else {
            string tempzeroname = "temp" + inttostr(innerCode.tempNum);
            ++innerCode.tempNum;
        ...
        if (node->left->right->right->name == "expression_statement") {
            ...
            if (node->left->right->right->right->right->name == ")") {
                ...
                if (exp_state2->left->name == "expression") {
                    ...
                    innerCode.addCode("(j, _, _, " + label2 + ")");
                }
        ...
        if (declarator->left->name == "IDENTIFIER") {
            ...
            //????????
                ...
                if (arrayType == "int") {
                    ...
                    innerCode.addCode("(=, #4, _, "+tempName3+")");

                    innerCode.addCode("(*, "+tempName3+", "+rnode.name+", "+tnode.name+")");
                }
                    ...
                    innerCode.addCode("(=, "+tempName3+", _, #8)");

                    innerCode.addCode("(*, "+tempName3+", "+rnode.name+", "+tnode.name+")");//"(*, "+tempName3+", "+rnode.name+", "+tnode.name+")"
                }
...
varNode Praser::praser_equality_expression(struct gramTree* equality_exp) {
    ...
    if (equality_exp->left->name == "relational_expression") {
        ...
        newnode.boolString = "j" + op + ", " + innerCode.getNodeName(node1) + ", " + innerCode.getNodeName(node2);
...
varNode Praser::praser_unary_expression(struct gramTree*unary_exp) {
  if (unary_exp->left->name == "postfix_expression") {
    ...
    if (rnode.useAddress) {
      ...
      innerCode.addCode("(+, *" + rnode.name + ", " + tempname + ", *" + rnode.name + ")" );
    }
    else {
      //innerCode.addCode(innerCode.getNodeName(rnode) + " := " + innerCode.getNodeName(rnode) + " + "  + tempname);
      innerCode.addCode("(+, " + innerCode.getNodeName(rnode) + ", " + tempname + ", " + innerCode.getNodeName(rnode) + ")" );
    }
    ...
    if (op == "+") {
      ...
      if (rnode.useAddress) {
        ...
        innerCode.addCode("(-, " + tempzeroname + ", *" + rnode.name + ", " + tempname + ")" );
      }
      else {
        //innerCode.addCode(tempname + " := " + tempzeroname + " - " + innerCode.getNodeName(rnode));
        innerCode.addCode("(-, " + tempzeroname + ", " + innerCode.getNodeName(rnode) + ", " + tempname + ")" );
      }
      return newnode;
    }
    else if (op == "~") {
  ...
  if (post_exp->left->name == "primary_expression") {
    ...
    if (anode.type == "int" || anode.type == "double") {
      ...
      if (anode.type == "int") {
        ...
        innerCode.addCode("(*, " + innerCode.getNodeName(enode) + ", " + tempName3 + ", " + tempName2 + ")");
      }
        ...
        innerCode.addCode("(=, #8, _, " + tempName3 + ")");
        innerCode.addCode("(*, " + innerCode.getNodeName(enode) + ", " + tempName3 + ", " + tempName2 + ")");
        //innerCode.addCode(tempName2 + " := " + innerCode.getNodeName(enode) + " * " + tempName3);
      }
      ...
      innerCode.addCode("(+, &" + innerCode.getarrayNodeName(anode) + ", " + innerCode.getNodeName(tempVar2) + ", " + tempName + ")");
      return tempVar;
    }


    //innerCode.addCode(tempName + " := &" + innerCode.getarrayNodeName(anode) + " + " + innerCode.getNodeName(enode));
    innerCode.addCode("(+, &" + innerCode.getarrayNodeName(anode) + ", " + innerCode.getNodeName(enode) + ", " + tempName + ")");
    return tempVar;
  }
  else if (post_exp->left->right->name == "(") {
    //????????
    ...
    innerCode.addCode("(=, #1, _, " + tempnameone + ")");
    ...
    if (rnode.useAddress) {
      ...
      innerCode.addCode("(+, *" + rnode.name + ", " + tempnameone + ", *" + rnode.name + ")");
    }
    else {
      ...
      innerCode.addCode("(+, " + innerCode.getNodeName(rnode) + ", " + tempnameone + ", " + innerCode.getNodeName(rnode) + ")");
    }
...
varNode Praser::praser_primary_expression(struct gramTree* primary_exp) {
  if (primary_exp->left->name == "IDENTIFIER") {
    ...
    varNode newNode = createTempVar(tempname, "int");
    blockStack.back().varMap.insert({ tempname,newNode });
    innerCode.addCode("(=, #" + content +", _, " + tempname + ")");
    return newNode;
  }
    ...
    blockStack.back().varMap.insert({ tempname,newNode});
    innerCode.addCode("(=, F" + content +", _, " + tempname + ")");
    return newNode;
  }
